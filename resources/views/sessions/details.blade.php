                    ...
                    <div class="row">
                            ...
                            <p class="text-muted">
                                ...
                                @if ($session->patient->PTNT_BLNC != 0)
                                    ...
                                    <small>Current Balance:
                                        <strong>{{ number_format($session->patient->PTNT_BLNC) }}</strong></small>
                                @endif
                            </p>
                        </div>
                        ...
                        <div class="col-md-2">
                            @if (Auth::user()->canAdmin())
                                ...
                                <p class="text-muted">{{ $session->SSHN_TOTL . ' EGP' }}
                                    {{ $session->discount > 0 ? '(' . $session->discount . ')' : '' }}</p>
                            @endif
                        </div>
                        <div class="col-md-2">
                            <div class="font-bold mb-2">
                        ...
                        @if ($patient_packages->count() > 0)
                            <div class="col-md-2">
                                ...
                                @foreach ($patient_packages as $item)
                                    <strong>
                                        <p class="text-muted">{{ $item->pricelistItem->item_name }} :
                                            {{ $item->PTPK_QNTY }}</p>
                                    </strong>
                                @endforeach
                ...
                <div class="card-header">
                    <ul class="nav nav-tabs card-header-tabs">
                        <li class="nav-item"> <a class="nav-link active" role="tab" data-toggle="tab"
                                href="#status">Session Status</a> </li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="status" role="tabpanel">
                        <div class="card-body">
                            ...
                            <ul>
                                ...
                                @else
                                    <li>
                                        <p class="text-muted">No Assigned Doctor yet <i
                                                class=" fas fa-exclamation-triangle" style="color:#fec107"></i></p>
                                    </li>
                                @endif
                                    ...
                                    <li>
                                        ...
                                        <ul>
                                            @foreach ($session->items as $item)
                                                <li>
                                                    <p class="text-muted">{{ $item->pricelistItem->device->DVIC_NAME }} -
                                    ...
                                    <li>
                                        @if ($session->remaining_money != 0)
                                            <p class="text-muted">Payment not yet fully collected please collect payment
                                                before setting Session as Done, {{ $session->remaining_money }}EGP
                                                remaining
                            ...
                            <button class="btn btn-success mr-2"
                                onclick="confirmAndGoTo('{{ url($setSessionConfirmedUrl) }}', 'Set Session as Confirmed')"
                                @if ($session->SSHN_CONF) disabled @endif>
                            ...
                            <button class="btn btn-info mr-2"
                                onclick="confirmAndGoTo('{{ url($setSessionNewUrl) }}', 'Set Session as New')"
                                @if (!$session->canBeNew()) disabled @endif>Set Session as
                                New</button>
                            ...
                            <button class="btn btn-success mr-2" onclick="setAsDone()"
                                @if (!$session->canBeDone()) disabled @endif>Set Session As Done</button>
                            ...
                            <form class="form pt-3" method="post" action="{{ url($updateServicesURL) }}">
                                ...
                                <div class="row ">
                                    <div class="mb-10 col-3 ">
                                        <button type="button" class="btn btn-success" onclick="addService()"
                                            @if (!$session->canEditServices()) disabled @endif>Add Service</button>
                                    </div>
                                    <div class="ml-10 col-9 d-flex justify-content-end">
                                        <div class="bt-switch">
                                            <div>
                                                <input type="checkbox" data-size="medium" data-on-color="success"
                                                    data-off-color="danger" data-on-text="Commission"
                                                    {{ $session->SSHN_CMSH ? 'checked' : '' }}
                                                    data-off-text="No Commission" name="isCommission"
                                ...
                                @foreach ($session->items as $item)
                                    <div class="row removeclass{{ $i }}">
                                        <div class="mr-2 adjust-self-center">
                                            @if ($item->is_collected)
                                                <i class="fas fa-check-circle" style="color:lightgreen"
                                                    title="Collected from Client Package"></i>
                                        ...
                                        <div class="col-lg-3">
                                            <div class="input-group mb-2">
                                                <select name="device[{{ $i }}]"
                                                    class="form-control select2 custom-select" style="width:100%"
                                                    id="device{{ $i }}"
                                                    onchange="loadServices({{ $i }})" required
                                                    @if (!$session->canEditServices()) disabled @endif>
                                                    <option disabled hidden selected value="" class="text-muted">
                                                        Device</option>
                                                    @foreach ($devices as $device)
                                                        <option value="{{ $device->id }}"
                                                            @if ($device->id == $item->deviceObj()->id) selected @endif>
                                                            {{ $device->DVIC_NAME }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-3">
                                            <div class="input-group mb-3">
                                                <input id="note{{ $i }}" value="{{ $item->SHIT_NOTE }}"
                                                    type="text" class="form-control" placeholder="Note"
                                                    name="note[{{ $i }}]">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-2">
                                            <div class="input-group mb-3">
                                                <input id="unit{{ $i }}" value="{{ $item->SHIT_QNTY }}"
                                                    type="number" step="0.01" class="form-control amount"
                                                    placeholder="Unit" name="unit[{{ $i }}]"
                                                    {{ (isset($readOnly) && $readOnly) || !$session->canEditServices() ? 'readonly' : '' }}>

                                                <div class="input-group-append">
                                                    <button class="btn btn-danger" type="button"
                                                        onclick="removeService({{ $i }});"
                                                        @if (!$session->canEditServices()) disabled @endif><i
                                ...
                                <button type="submit" class="btn btn-success mr-2"
                                    @if (!$session->canEditServices()) disabled @endif>Submit</button>
                            </form>
                    ...
                    <div class="tab-pane" id="history" role="tabpanel">
                        <div class="card-body">
                            ...
                            <div class="col-12">
                                <x-datatable id="patientServicesTable" :title="'Services History'" :subtitle="$subTitle ?? ''"
                    ...
                    @if (Auth::user()->canAdmin())
                        ...
                        <div class="tab-pane" id="payment" role="tabpanel">
                            <div class="card-body">
                                ...
                                <h6 class="card-subtitle">Total: {{ $session->SSHN_TOTL }} - Paid:
                                    {{ $session->SSHN_PAID }} - Client Balance: {{ $session->SSHN_PTNT_BLNC }} - Discount:
                                    {{ $session->discount }}
                                    ({{ $session->SSHN_DISC }}%) -
                                ...
                                @if ($session->canEditMoney())
                                    <form class="form pt-3" method="post" action="{{ url($paymentURL) }}">
                                        ...
                                        <div class="row">
                                            <div class="col-9">
                                                <div class="form-group">
                                                    <label>Payment</label>
                                                    <div class="input-group mb-3">
                                                        <input type="number" step=.01 class="form-control amount"
                                                            placeholder="Items Count" min=0.01 name=amount value="0"
                                                            required>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <button type="submit" class="btn btn-success mr-2">Collect Normal
                                            Payment</button>
                                        <button type="button" class="btn btn-success mr-2"
                                            onclick="confirmAndGoTo('{{ url($settleSessionOnPackagesURL) }}', 'Settle Session on Client Packages')"
                                            @if ($session->remaining_money <= 0) disabled @endif>Settle Session on Client
                                            Packages</button>
                                        <button type="button" class="btn btn-success mr-2"
                                            onclick="confirmAndGoTo('{{ url($settleSessionOnBalanceURL) }}', 'Settle Session on Client Balance')"
                                            @if ($session->remaining_money <= 0) disabled @endif>Settle Session on Client
                                            Balance</button>
                                        <p class="text-muted mt-2">Payments higher than
                                            <strong>{{ $session->remaining_money }}</strong> will be added to the Patient
                                            Balance</p>
                                    </form>
                                    <hr>
                                    <form class="form pt-3" method="post" action="{{ url($discountURL) }}"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group mb-3">
                                                <input type="number" step=1 class="form-control amount" min=0
                                                    max={{ $session->remaining_discount }} name=discount
                                                    value="{{ $session->SSHN_DISC }}" required>
                                            </div>
                                            <p class="text-muted mt-2">Enter discount percentage value (0-100)</p>
                                        </div>
                                        <button type="submit" class="btn btn-success mr-2"
                                            @if ($session->remaining_money <= 0) disabled @endif>Set Discount</button>
                                    </form>
                                @else
                                ...
                                <form class="form pt-3" method="post" action="{{ url($doctorURL) }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2 form-control  col-md-12 mb-3" style="width:100%"
                                            name=doctorID required @if (!$session->canEditDoctor()) disabled @endif>
                                            @foreach ($doctors as $doctor)
                                                <option value="{{ $doctor->id }}"
                                                    @if ($doctor->id == $session->SSHN_DCTR_ID) selected @endif>
                                                    {{ $doctor->DASH_USNM }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <button type="submit" class="btn btn-success mr-2"
                                        @if (!$session->canEditDoctor()) disabled @endif>Set Doctor</button>
                        ...
                        <div class="tab-pane " id="stock" role="tabpanel">
                            <div class="card-body">
                                <div class="card-body">
                                    ...
                                    <div class="col-12">
                                        <x-datatable id="stockTable" :title="'Stock'" :subtitle='''' :cols="$stockCols"
                                            :items="$stockTrans" :atts="$stockAtts" :cardTitle="false" />
                                    </div>
                                                    ...
                                                    `<div class="row col-lg-12">
                                                            ...
                                                            <div class="col-lg-6">
                                                                <div class="input-group mb-3">
                                                                    ...
                                                                    <div class="input-group-append">
                                                                        <button class="btn btn-danger" id="dynamicAddButton" type="button" onclick="removeToab( ` +
                                                    stockaya + ` );"><i class="fa fa-minus"></i></button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div`;
                                ...
                                <div class="card-body">
                                    ...
                                    @if ($session->canEditInfo())
                                        <form class="form pt-3" method="post" action="{{ url($editSessionURL) }}">
                                            ...
                                            <div class="form-group">
                                                <label>Date*</label>
                                                <div class="input-group mb-3">
                                                    <input type="date" class="form-control" placeholder="Session Day"
                                                        name=sessionDate
                                                        value="{{ $session->SSHN_DATE->format('Y-m-d') }}" required>
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group mb-3">
                                                    <input type="time" class="form-control"
                                                        ...
                                                        value="{{ $session->carbon_start_time->format('H:i A') }}"
                                                        name=sessionStartTime required>
                                                </div>
                                                <small
                                                    class="text-danger">{{ $errors->first('sessionStartTime') }}</small>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group mb-3">
                                                    <input type="time" class="form-control"
                                                        ...
                                                        value="{{ $session->carbon_end_time->format('H:i A') }}"
                                                        name=sessionEndTime required>
                                                </div>
                                    ...
                                    @if ($canDelete)
                                        <button type="button" class="btn btn-danger mr-2"
                                            onclick="confirmAndGoTo('{{ url($deleteSessionURL) }}', 'delete this session and all its info')">Delete
                                            Session</button>
                                    @endif
                                </div>
                            </div>
        ...
        @if ($session->canEditServices())
            ...
            function addService() {
                ...
                concatString += '<div class="col-lg-3">\
                                        <div class="input-group mb-2">\
                                            <select name=device[' + room +
                    '] class="form-control select2 custom-select" style="width:100%" id="device' + room +
                    '" onchange="loadServices(' + room +
                    ')" required>\
                ...
                concatString += '<div class="col-2">\
                                <div class="input-group mb-2">\
                                    <select name=service[' + room +
                    '] class="form-control select2 custom-select" style="width:100%" id="service' + room +
                    '" onchange="checkUnit(' + room + ')" disabled required>\
                                    </select>\
                                </div>\
                            </div>'
