                            ...
                            @foreach ($atts as $att)
                                @if (is_array($att))
                                    ...
                                    @elseif(array_key_exists('editJS', $att))
                                        <td>
                                            <div onclick="{{ $att['editJS']['func'] }}({{ $item->id }})"><a
                                                    href="javascript:void(0)"><img src="{{ asset('images/edit.png') }}"
                                    ...
                                    @elseif(array_key_exists('del', $att))
                                        <td>
                                            <div
                                                onclick="confirmAndGoTo('{{ url($att['del']['url'] . $item->{$att['del']['att']}) }}', '{{ $att['del']['msg'] ?? 'delete this row' }}')">
                                                <a href="javascript:void(0)"><img src="{{ asset('images/del.png') }}"
                                                        width=25 height=25></a>
                                            </div>
                                        </td>
                                    ...
                                    @elseif(array_key_exists('foreignDate', $att))
                                        <td>{{ $item->{$att['foreignDate']['rel']}->{$att['foreignDate']['att']}->format($att['foreignDate']['format']) ?? '' }}
                                        </td>
                                        ...
                                        <td>{{ $item->{$att['verifiedRel']['rel']}->{$att['verifiedRel']['relAtt']} ?? '' }}
                                            @isset($item->{$att['verifiedRel']['rel']}->{$att['verifiedRel']['relAtt']})
                                                @if ($item->{$att['verifiedRel']['isVerified']})
                                                    <i title="{{ $att['verifiedRel']['iconTitle'] }}"
                                                        class="fas fa-check-circle" style="color:lightgreen"></i>
                                                @else
                                                    <i title="{{ $att['verifiedRel']['iconTitle'] }}"
                                    ...
                                    @elseif(array_key_exists('dynamicUrl', $att))
                                        <td><a
                                                href="{{ url($att['dynamicUrl']['baseUrl'] . $item->{$att['dynamicUrl']['val']}) }}">{{ $item->{$att['dynamicUrl']['att']} }}</a>
                                        </td>
                                    @elseif(array_key_exists('state', $att))
                                        <td>
                                            @if (array_key_exists('url', $att['state']))
                                                <a href="{{ url($att['state']['url'] . $item->{$att['state']['urlAtt']}) }}"
                                                    target="_blank">
                                            @endif
                                            <button
                                                class="label {{ $att['state']['classes'][$item->{$att['state']['att']}] }}">{{ $att['state']['text'][$item->{$att['state']['att']}] }}</button>
                                            @if (array_key_exists('url', $att['state']))
                                                </a>
                                            @endif
                                        </td>
                                    @elseif(array_key_exists('leadState', $att))
                                        <td>

                                            <a class="openLeadStatus" href="javascript:void(0)" data-toggle="modal"
                                                data-target="#set-lead-status" data-id="{{ $item->id }}">

                                                <button
                                                    class="label {{ $att['leadState']['classes'][$item->{$att['leadState']['att']}] }}">{{ $att['leadState']['text'][$item->{$att['leadState']['att']}] }}</button>
                                    ...
                                    @elseif(array_key_exists('foreignUrl', $att))
                                        <td><a
                                                href="{{ url($att['foreignUrl']['baseUrl'] . '/' . $item->{$att['foreignUrl']['urlAtt']}) }}">{{ $item->{$att['foreignUrl']['rel']}->{$att['foreignUrl']['att']} }}</a>
                                        </td>
                                    @elseif(array_key_exists('assetImg', $att))
                                        <td>
                                            @isset($item->{$att['assetImg']['att']})
                                                <img src="{{ asset('storage/' . $item->{$att['assetImg']['att']}) }}"
                                    ...
                                    @elseif(array_key_exists('comment', $att))
                                        <td>
                                            <button type="button" style="padding:.1rem" class="btn btn-secondary"
                                                ...
                                                <div style="display: none">{{ $item->{$att['comment']['att']} }}</div>
                                                <i class="far fa-list-alt"></i>
                                            </button>
                                        </td>
                                    @elseif(array_key_exists('hidden', $att))
                                        <input type=hidden id="{{ $att['hidden']['id'] }}{{ $item->id }}"
                                            value="{{ $item->{$att['hidden']['valueAtt']} }}" />
